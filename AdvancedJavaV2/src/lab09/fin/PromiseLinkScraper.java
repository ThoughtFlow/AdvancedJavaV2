...
public class PromiseLinkScraper {
	...
	private static Map<String, Integer> invoke(List<String> urls) throws InterruptedException, ExecutionException {
		...
		for (String nextUrl : urls) {
			CompletableFuture<Map<String, Integer>> nextPromise = CompletableFuture.supplyAsync(() -> Util.scrapeHrefs(nextUrl)).thenApply(list -> Util.catalog(list));
		   
		    promise = promise.thenCombine(nextPromise, (first, second) -> Util.merge(first, second));
		}
