...
public class StreamGrep {
	...
	private static long grepDashCWithReduce(Stream<String> in, String upperCaseSearchWord) {

		// The long way
		return in.map(String::toUpperCase).filter(s -> s.contains(upperCaseSearchWord)).mapToLong(count -> 1).reduce(0, (l, r) -> l + r);
	}

	private static long grepDashCWithCount(Stream<String> in, String upperCaseSearchWord) {

		// Using the built-in function count
		return in.map(String::toUpperCase).filter(s -> s.contains(upperCaseSearchWord)).count();
	}
...
//		}
		...
		try (BufferedReader bufferedReader = Util.getReader(url)) {
			System.out.println("Using grepDashCWithReduce - Found: " + grepDashCWithReduce(bufferedReader.lines(), wordSearch));
		}
		catch (IOException e) {
			e.printStackTrace();
		}
